...
const ejecutarComando = async () => {
    ...
    if (normalizar(comando.value) === normalizar(consultaCorrecta.value)) {
        ...
        setTimeout(() => {
            ...
            router.push({ path: `/mision/4` }); // 👈 solo esto
        }, 2500);
        try {
...
onMounted(async () => {
    ...
    try {
        ...
        // 📘 Obtener misión
        const resMision = await fetch('https://command-sql-back.onrender.com/misiones/3', {
            headers: { Authorization: `Bearer ${token}` }
        });
        ...
        const resNivel = await fetch(`https://command-sql-back.onrender.com/niveles/3`, {
            headers: { Authorization: `Bearer ${token}` }
        });
...
<template>
    <div :class="$style.mainscreen">
        ...
        <div :class="$style.background">
            <img src="@/assets/12.png" alt="Fondo" />
        ...
        <div :class="$style.cardParent">
            <div :class="$style.card">
                <div :class="$style.commandSql">MISION 3 — {{ nivelTitulo }}</div>
            </div>
    ...
    <div v-if="mostrarPopup" :class="$style.popupOverlay">
        <div :class="$style.popup">
            ...
            <p>Has superado esta misión con éxito, adelante cadete..</p>
