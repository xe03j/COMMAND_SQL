...
const ejecutarComando = async () => {
    ...
    if (normalizar(comando.value) === normalizar(consultaCorrecta.value)) {
        ...
        setTimeout(() => {
            ...
            router.push({ path: `/mision/14` }); // 👈 solo esto
        }, 2500);
        try {
...
onMounted(async () => {
    ...
    try {
        ...
        // 📘 Obtener misión
        const resMision = await fetch('https://command-sql-back.onrender.com/misiones/13', {
            headers: { Authorization: `Bearer ${token}` }
        });
...
<template>
    <div :class="$style.mainscreen">
        ...
        <div :class="$style.background">
            <img src="@/assets/20.png" alt="Fondo" />
        ...
        <div :class="$style.cardParent">
            <div :class="$style.card">
                <div :class="$style.commandSql">MISION 13 — {{ nivelTitulo }}</div>
            </div>
