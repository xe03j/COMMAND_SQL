    ...
    if (normalizar(comando.value) === normalizar(consultaCorrecta.value)) {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                router.push({ path: `/dashboard` }); // 👈 solo esto
            }, 2500);
...
onMounted(async () => {
    ...
    try {
        ...
        // 📘 Obtener misión
        const resMision = await fetch('https://command-sql-back.onrender.com/misiones/16', {
            headers: { Authorization: `Bearer ${token}` }
        });
...
<template>
    <div :class="$style.mainscreen">
        ...
        <div :class="$style.background">
            <img src="@/assets/21.png" alt="Fondo" />
        ...
        <div :class="$style.cardParent">
            <div :class="$style.card">
                <div :class="$style.commandSql">MISION 16 — {{ nivelTitulo }}</div>
            </div>
