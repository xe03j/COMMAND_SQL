...
const ejecutarComando = async () => {
    ...
    if (normalizar(comando.value) === normalizar(consultaCorrecta.value)) {
        ...
        setTimeout(() => {
            ...
            router.push({ path: `/mision/6` }); // 👈 solo esto
        }, 2500);
...
onMounted(async () => {
    ...
    try {
        ...
        // 📘 Obtener misión
        const resMision = await fetch('https://command-sql-back.onrender.com/misiones/5', {
            headers: { Authorization: `Bearer ${token}` }
        });
    ...
    <div :class="$style.mainscreen">
        ...
        <div :class="$style.cardParent">
            <div :class="$style.card">
                <div :class="$style.commandSql">MISION 5 — {{ nivelTitulo }}</div>
            </div>
