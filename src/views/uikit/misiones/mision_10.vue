...
const ejecutarComando = async () => {
    ...
    if (normalizar(comando.value) === normalizar(consultaCorrecta.value)) {
        ...
        setTimeout(() => {
            ...
            router.push({ path: `/mision/11` }); // 👈 solo esto
        }, 2500);
...
onMounted(async () => {
    ...
    try {
        ...
        // 📘 Obtener misión
        const resMision = await fetch('https://command-sql-back.onrender.com/misiones/10', {
            headers: { Authorization: `Bearer ${token}` }
        });
    ...
    <div :class="$style.mainscreen">
        ...
        <div :class="$style.cardParent">
            <div :class="$style.card">
                <div :class="$style.commandSql">MISION 10 — {{ nivelTitulo }}</div>
            </div>
