...
const ejecutarComando = async () => {
    ...
    if (normalizar(comando.value) === normalizar(consultaCorrecta.value)) {
        ...
        setTimeout(() => {
            ...
            router.push({ path: `/mision/5` }); // 👈 solo esto
        }, 2500);
        try {
...
onMounted(async () => {
    ...
    try {
        ...
        const resNivel = await fetch(`https://command-sql-back.onrender.com/niveles/4`, {
            headers: { Authorization: `Bearer ${token}` }
        });
    ...
    <div :class="$style.mainscreen">
        ...
        <div :class="$style.cardParent">
            <div :class="$style.card">
                <div :class="$style.commandSql">MISION 4 — {{ nivelTitulo }}</div>
            </div>
